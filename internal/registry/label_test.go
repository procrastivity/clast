...
// Ported from wip's internal/tiers/label_test.go TestDefaultLabel.
...
// first tier: the parent directory's name. Ported from wip's
// internal/tiers/label_test.go TestSuggestLabel_ParentDirTier.
...
// parent-dir name is also taken. Ported from wip's
// internal/tiers/label_test.go TestSuggestLabel_BasenameParentTier.
...
// TestSuggestLabel_Exhausted asserts M16's "no -2, -3, ever": when both
// tiers collide, SuggestLabel reports no suggestion rather than inventing
// a third form. Ported from wip's internal/tiers/label_test.go
// TestSuggestLabel_Exhausted.
...
// does not depend on registration order — only the path and the current
// taken set. Ported from wip's internal/tiers/label_test.go
// TestSuggestLabel_PureFunctionOfPathAndTakenSet.
