...
// (CONTRACT.md C3): the flat JSON object `clast manifest --json` emits —
...
// names, the same package path — so `clast version` and
// `clast manifest --json`'s tool object are never two sources for one
// fact.
type Tool struct {
...
// Manifest is the flat object `clast manifest --json` emits.
