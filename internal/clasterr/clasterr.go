// Package clasterr is the structured error type every verb raises for a
...
// "clast: <verb>: <message>" line in human mode, or the {"error": {"code",
...
func Render(w io.Writer, verb string, err *Error, jsonMode bool) {
	...
	if verb == "" {
		_, _ = fmt.Fprintf(w, "clast: %s\n", err.Message)
		return
	}
	_, _ = fmt.Fprintf(w, "clast: %s: %s\n", verb, err.Message)
}
