// Package manifest implements the `clast manifest` verb: the tool's own
...
// Command constructs the `clast manifest` verb. root is the same
...
func Command(streams *iostreams.Streams, build buildinfo.Info, root *cobra.Command) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "manifest",
		Short: "print clast's machine-readable self-description: verbs, kinds, and shipped assets",
