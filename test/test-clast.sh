...
_assert_skill_day_wakeup_frontmatter() {
  ...
  if [[ "$(head -1 "$skill")" != '---' ]]; then
    printf 'day-wakeup SKILL.md: must begin with ---\n' >&2
    ok=1
  fi
  if ! awk '/^---$/{n++; if(n==2){found=1; exit}} END{exit !found}' \
       <(head -50 "$skill"); then
    printf 'day-wakeup SKILL.md: closing --- not found within first 50 lines\n' >&2
    ok=1
  fi
  if ! grep -q '^name: day-wakeup$' "$skill"; then
    printf 'day-wakeup SKILL.md: missing name: day-wakeup\n' >&2
    ok=1
  fi
  local key_count
  key_count=$(awk '/^---$/{n++; next} n==1 && /^[A-Za-z_-]+:/{c++} END{print c+0}' "$skill")
  if [[ "$key_count" -ne 2 ]]; then
    printf 'day-wakeup SKILL.md: frontmatter must contain exactly two keys (found %d)\n' "$key_count" >&2
    ok=1
  fi
  local desc_line
  desc_line=$(grep -c '^description:' "$skill" || true)
  if [[ "$desc_line" -lt 1 ]]; then
    printf 'day-wakeup SKILL.md: missing description field\n' >&2
    ok=1
  fi
  local desc_len
  desc_len=$(awk '/^description:/{p=1} p{s=s $0} /^---$/ && NR>1{p=0} END{print length(s)}' "$skill")
  if [[ "$desc_len" -lt 100 ]]; then
    printf 'day-wakeup SKILL.md: description field too short (%d chars)\n' "$desc_len" >&2
    ok=1
  fi
  return "$ok"
}
...
_assert_skill_day_wakeup_triggers() {
  ...
  for phrase in '/day-wakeup' 'morning briefing' 'catch me up on yesterday'; do
    if ! grep -q "$phrase" "$skill"; then
      printf 'day-wakeup SKILL.md: missing trigger phrase: %s\n' "$phrase" >&2
      ok=1
    fi
  done
  return "$ok"
...
_assert_skill_day_wakeup_cli_commands() {
  local skill=.claude-plugin/skills/day-wakeup/SKILL.md
  local ok=0
  for cmd in 'clast snapshot' 'clast sessions --day yesterday' 'clast show' \
...
_assert_plugin_assets() {
  ...
  printf 'plugin asset check: day-wakeup/SKILL.md frontmatter\n'
  if ! _assert_skill_day_wakeup_frontmatter; then ok=1; fi
  printf 'plugin asset check: day-wakeup/SKILL.md trigger phrases\n'
  if ! _assert_skill_day_wakeup_triggers; then ok=1; fi
  printf 'plugin asset check: day-wakeup/SKILL.md CLI commands\n'
  if ! _assert_skill_day_wakeup_cli_commands; then ok=1; fi
  return "$ok"
}
